...
public class HostUpdateController {
    ...
    private void initDefaultValues() {
        txtHostAddress.setText(host.getAddress());
        chkSSL.setSelected(host.isUseSSL());
        txtCertificatesPath.setText(host.getCertificatesPath());

        if (CollectionUtils.isNotEmpty(host.getHostVariables())) {
            hostVariables.addAll(host.getHostVariables());
        }
    }
    ...
    public void save(ActionEvent actionEvent) {
        ...
        Host newHost = new Host(txtHostAddress.getText(), chkSSL.isSelected(), txtCertificatesPath.getText(), hostVariables);
        Context.project.getHosts().remove(host);
        Context.project.getHosts().add(newHost);
        close();
    }
